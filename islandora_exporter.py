...
def fetch_children(collection_pid, parent_directory):
    ...
    for child in children:
        if child['RELS_EXT_hasModel_uri_t'] = 'info:fedora/islandora:collectionCModel':
            ...
            obj_content_path = os.path.join(parent_directory, (child['fgs_label_t'] + extension))
            with open(obj_content_path, 'wb') as file:
                file.write(obj.content)

        print("Downloading " +  site_base_url + "islandora/object/" + object['PID'])
...
if __name__ == "__main__"
    fetch_children("islandora%3Aroot", output_directory)
